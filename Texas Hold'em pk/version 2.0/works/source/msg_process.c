...
FloatToStr((float)g_Money_Max , "Oppon_Bet_Infos:", NULL);
...
FloatToStr(g_winpro, "River Win Pro:  ",NULL);
write(debug_fd,g_Str,strlen(g_Str));//
	}
	else if(Card_Null != Public_Cards[3].card_val)//转牌圈
	{
		...
		pact_str = g_Action_Str[Idx];
FloatToStr(g_winpro, "Turn Win Pro:  ",NULL);
write(debug_fd,g_Str,strlen(g_Str));// 
	}
	else if(Card_Null != Public_Cards[0].card_val)//翻牌圈
	{
		...
		pact_str = g_Action_Str[Idx];
FloatToStr(g_winpro, "Flop Win Pro:  ",NULL);
write(debug_fd,g_Str,strlen(g_Str));// 
	}
...
FloatToStr((float)Sklansky_Val, "Hand Stage: ",NULL);
write(debug_fd,g_Str,strlen(g_Str));// 
	}
