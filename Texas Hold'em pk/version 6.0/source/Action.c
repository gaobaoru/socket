...
#define MONEY_M 0.1*g_now_jetton
#define MONEY_N 0.08*g_now_jetton
#define MONEY_K 0.05*g_now_jetton
#define MONEY_H 0.03*g_now_jetton
...
uint16_t g_now_jetton=0;//每局更新所剩筹码 
...
//printf("取胜概率: %f\n",winpro);
	...
	if(winpro >= 0.8)
	{
		if(g_Money_Max >= 2*MONEY_M)//自己需要下注额大于800时，弃牌 
		{
			return Fold; 
		}
		return Call;
