...
void Updata_Opponent_Noti_ActionNum(void)
{
	...
	for(i=0; i<8; i++)
	{
		for(j=0; j<8; j++)
		{
			if(notify_action[i].id == opponent_action[j].id && notify_action[i].action == 1)
			{
...
write(debug_fd,g_Str,strlen(g_Str));
			...
			else if(notify_action[i].id == opponent_action[j].id && notify_action[i].action == 2)
				opponent_action[j].opponent_call++;
			else if(notify_action[i].id == opponent_action[j].id && notify_action[i].action == 3)
				opponent_action[j].opponent_raise++;
			else if(notify_action[i].id == opponent_action[j].id && notify_action[i].action == 4)
				opponent_action[j].opponent_all_in++;
			else if(notify_action[i].id == opponent_action[j].id && notify_action[i].action == 5)
...
void Updata_Opponent_Inqu_ActionNum(void)
{
	...
	for(i=0; i<8; i++)
	{
		for(j=0; j<8; j++)
		{
			if(inquire_action[i].id == opponent_action[j].id && inquire_action[i].action == 1)
			{
				opponent_action[j].opponent_fold++;
FloatToStr((float)j,"the order :", NULL);
write(debug_fd,g_Str,strlen(g_Str));
FloatToStr((float)inquire_action[i].action,"inquire_action :", NULL);
write(debug_fd,g_Str,strlen(g_Str));
FloatToStr((float)opponent_action[j].opponent_fold,"Inquire_fold_ActionNum:", NULL);
write(debug_fd,g_Str,strlen(g_Str));
			}
			else if(inquire_action[i].id == opponent_action[j].id && inquire_action[i].action == 2)
				opponent_action[j].opponent_call++;
			else if(notify_action[i].id == opponent_action[j].id && inquire_action[i].action == 3)
				opponent_action[j].opponent_raise++;
			else if(inquire_action[i].id == opponent_action[j].id && inquire_action[i].action == 4)
				opponent_action[j].opponent_all_in++;
			else if(inquire_action[i].id == opponent_action[j].id && inquire_action[i].action == 5)
				opponent_action[j].opponent_check++;
		}
	}
}
