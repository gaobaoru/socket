...
std::string ResourceHost::lookupMimeType(std::string ext){
    std::unorered_map<std::string, std::string>::const_iterator itor = mimeType.find(ext);
    if(itor == mimeType.end())
	return "text/html";
...
Resource* ResourceHost::readFile(std::string path, struct stat sb){
    ...
    file.open(path.c_str(), std::ios::library);
